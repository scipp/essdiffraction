...
def test_pdf_structure_factor_needs_q_coord():
    ...
    with pytest.raises(KeyError):
        compute_pdf_from_structure_factor(
            da, sc.scalar(1.0, unit='1/angstrom'), r, use_filter=False
        )


@pytest.mark.parametrize('use_filter', [(False,), (True,)])
def test_pdf_structure_factor(use_filter):
    da = sc.DataArray(
    ...
    v = compute_pdf_from_structure_factor(
        da, sc.scalar(1.0, unit='1/angstrom**3'), r, use_filter=use_filter
    )
...
def test_pdf_structure_factor_result_unchanged():
    ...
    r = sc.array(dims='r', values=[2, 3, 4, 5.0], unit='angstrom')
    v = compute_pdf_from_structure_factor(
        da, sc.scalar(1.0, unit='1/angstrom**3'), r, use_filter=True
    )
    sc.testing.assert_allclose(
        v.data,
        sc.array(dims='r', values=[-4.25706, -0.528801, -5.29488], unit='1/angstrom'),
        rtol=sc.scalar(1e-5),
    )
